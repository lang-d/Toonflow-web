...
export const normalizeMusicCueAsset = (value: any): MusicCueAsset => ({ ...(value ?? {}) });
...
export const uploadMusicCueAsset = (params: { projectId: number; cueId: number; base64Data: string; name: string }) => axios.post("/production/music/cue/asset/upload", params).then((response) => unwrap<{ musicCueAsset: MusicCueAsset }>(response)).then((data) => normalizeMusicCueAsset(data.musicCueAsset));
...
export const uploadMusicLibraryVersion = (params: { projectId: number; editionId: number; base64Data: string; name: string }) => axios.post("/production/music/library/version/upload", params).then((response) => unwrap<{ libraryVersion: MusicLibraryVersion }>(response)).then((data) => normalizeMusicVersion(data.libraryVersion));
